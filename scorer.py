...
class Scorer:
    ...
    def compute_class_probabilities(self, labels: ArrayLike) -> Dict[Any, float]:
        ...
        total_count = sum(label_counts.values()) + self.alpha * len(self.class_labels)
        class_probabilities = {
            label: (label_counts[label] + self.alpha)/ total_count for label in self.class_labels
        }
        # <<< END OF YOUR CODE <<<
    ...
    def chi_square_gain(self, data: ArrayLike, labels: ArrayLike, split_attribute: int) -> float:
        ...
        for value in unique_values:
            ...
            for label in label_classes:
                value_counts.append(label_counter[label])
            contingency_table.append(value_counts)
            # <<< END OF YOUR CODE <<<
    ...
    def subset_data(self, data: ArrayLike, labels: ArrayLike, split_attribute: int, split_value: Any) -> Tuple[ArrayLike, ArrayLike]:
        ...
        #print("Data 的形状:", data.shape)
        #print("Split_attribute的值是：", split_attribute)
        #print("splite_value的值是:", split_value)
        mask = (data[:, split_attribute] == split_value)
        #print("Mask的形状:", mask.shape)
        data_subset = data[mask]
        #("Data_subset的形状:", data_subset.shape)
    ...
    def split_on_best(self, data: ArrayLike, labels: ArrayLike, exclude: Set=set()) -> Tuple[int, Dict[Any, Tuple[ArrayLike, ArrayLike]]]:
        ...
        for value in unique_values:
            #print("best_feature的形状:", best_feature)
            subsets[value] = self.subset_data(data, labels, best_feature, value)
        #print("best_feature=", best_feature, type(best_feature))
    ...
    def information_gain(self, data: ArrayLike, labels: ArrayLike, split_attribute: int) -> float:
        ...
        for value in np.unique(data[:, split_attribute]):
            subset_data, subset_labels = self.subset_data(data, labels, split_attribute, value)
            extropy_after += (len(subset_labels) / total_count) * self.information_score(subset_labels)
    ...
    def gini_gain(self, data: ArrayLike, labels: ArrayLike, split_attribute: int) -> float:
        ...
        for value in np.unique(data[:, split_attribute]):
            subset_data, subset_labels = self.subset_data(data, labels, split_attribute, value)
            gini_after += (len(subset_labels) / total_count) * self.gini_score(subset_labels)
